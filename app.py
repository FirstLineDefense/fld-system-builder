...
def build_hydraulic_intelligence_html(primary):
    ...
    html += f"<p><strong>Hydraulic Health:</strong> {intelligence.get('health', 'Not Available')}</p>"
    html += f"<p><strong>Critical:</strong> {intelligence.get('critical_count', 0)} | "
    html += f"<strong>Warnings:</strong> {intelligence.get('warning_count', 0)} | "
    html += f"<strong>Info:</strong> {intelligence.get('info_count', 0)}</p>"

    html += "<h3>Priority Recommendations</h3>"
    html += "<table>"
    html += "<tr><th>Priority</th><th>Title</th><th>Recommendation</th></tr>"
    ...
    html += "<table>"
    html += "<tr><th>Branch</th><th>Current Pipe</th><th>Recommended Pipe</th><th>Reason</th><th>Estimated Velocity</th></tr>"
    ...
    if pipe_recs:
        for item in pipe_recs:
            ...
            html += f"<td>{item['estimated_velocity_fps']:.2f} ft/s</td>"
            html += "</tr>"
    else:
        html += "<tr><td colspan='5'>No pipe recommendations generated.</td></tr>"
    ...
    html += "<table>"
    html += "<tr><th>Branch</th><th>Reason</th><th>Recommendation</th></tr>"

    split_recs = intelligence.get("branch_split_recommendations", [])
    ...
    if split_recs:
        ...
        html += "<tr><td colspan='3'>No branch split recommendations generated.</td></tr>"

    html += "</table>"

    html += "<h3>Hydraulic Warnings</h3>"
    html += "<table>"
    html += "<tr><th>Severity</th><th>Category</th><th>Message</th><th>Recommendation</th></tr>"
    ...
    html += "<table>"
    html += "<tr><th>Branch</th><th>Velocity ft/s</th><th>Flow GPM</th><th>Pressure Margin PSI</th></tr>"
